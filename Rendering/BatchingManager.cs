...
public class BatchingManager
{
	...
	{
		Batcher batcher;
		if (batcherType == Batcher.BatcherType.Sprite) batcher = new SpriteBatcher(10000, material, texture);
		else batcher = new SpriteSheetBatcher(10000, material, texture);
	...
	{
		if (batchers.ContainsKey(textureID) == false) CreateBatcherForTexture(renderer.material, renderer.texture, Batcher.BatcherType.Sprite);
	...
	{
		if (batchers.ContainsKey(textureID) == false) CreateBatcherForTexture(renderer.material, renderer.texture, Batcher.BatcherType.SpriteSheet);
	...
	{
		if (batchers.ContainsKey(textureID) == false) return;
